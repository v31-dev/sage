        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="worker in workers" :key="worker.hostname" class="hover:bg-muted/50">
              ...
              <!-- IP -->
              <TableCell class="hidden sm:table-cell font-mono text-xs text-muted-foreground truncate">{{ worker.ip }}</TableCell>

              <!-- CPU Bar -->
              <TableCell>
                <div v-if="workerMetrics[worker.hostname]" class="space-y-1">
                  <div class="flex justify-between items-center mb-1">
                    <span class="text-xs font-semibold" :class="getStatusTextColor(workerMetrics[worker.hostname].cpu)">
                      {{ Math.round(workerMetrics[worker.hostname].cpu * 10) / 10 }}%
                    </span>
                  </div>
                  <div class="w-full bg-muted rounded-full h-1.5">
                    <div 
                      :class="['h-1.5 rounded-full transition-all duration-300', getStatusColor(workerMetrics[worker.hostname].cpu)]"
              ...
              <TableCell>
                <div v-if="workerMetrics[worker.hostname]" class="space-y-1">
                  <div class="flex justify-between items-center mb-1">
                    <span class="text-xs font-semibold" :class="getStatusTextColor((workerMetrics[worker.hostname].mem / workerMetrics[worker.hostname].memTotal) * 100)">
                      {{ Math.round((workerMetrics[worker.hostname].mem / workerMetrics[worker.hostname].memTotal) * 10) / 10 }}%
                    </span>
                  </div>
                  <div class="w-full bg-muted rounded-full h-1.5">
                    <div 
                      :class="['h-1.5 rounded-full transition-all duration-300', getStatusColor((workerMetrics[worker.hostname].mem / workerMetrics[worker.hostname].memTotal) * 100)]"
                      :style="{ width: Math.min((workerMetrics[worker.hostname].mem / workerMetrics[worker.hostname].memTotal) * 100, 100) + '%' }"
                    ></div>
                  </div>
                </div>
                <div v-else class="text-xs text-muted-foreground">-</div>
              ...
              <TableCell class="hidden sm:table-cell">
                <div v-if="workerMetrics[worker.hostname]" class="space-y-1">
                  <div class="flex justify-between items-center mb-1">
                    <span class="text-xs font-semibold" :class="getStatusTextColor((workerMetrics[worker.hostname].disk / workerMetrics[worker.hostname].diskTotal) * 100)">
                      {{ Math.round((workerMetrics[worker.hostname].disk / workerMetrics[worker.hostname].diskTotal) * 10) / 10 }}%
                    </span>
                  </div>
                  <div class="w-full bg-muted rounded-full h-1.5">
                    <div 
                      :class="['h-1.5 rounded-full transition-all duration-300', getStatusColor((workerMetrics[worker.hostname].disk / workerMetrics[worker.hostname].diskTotal) * 100)]"
                      :style="{ width: Math.min((workerMetrics[worker.hostname].disk / workerMetrics[worker.hostname].diskTotal) * 100, 100) + '%' }"
                    ></div>
                  </div>
                </div>
                <div v-else class="text-xs text-muted-foreground">-</div>
              ...
              <TableCell class="hidden md:table-cell">
                <div v-if="workerMetrics[worker.hostname]" class="text-xs space-y-0.5">
                  <p class="font-mono"><span class="text-blue-600 dark:text-blue-400">↓</span> {{ workerMetrics[worker.hostname].netRx }} Mbps</p>
                  <p class="font-mono"><span class="text-orange-600 dark:text-orange-400">↑</span> {{ workerMetrics[worker.hostname].netTx }} Mbps</p>
                </div>
                <div v-else class="text-xs text-muted-foreground">-</div>
              </TableCell>
