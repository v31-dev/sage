  ...
  <Dialog v-model:open="isDeploymentLogsDialogOpen" :key="props.container.worker.hostname">
    <DialogTrigger asChild>
      <Button
        ...
        variant="outline"
        size="sm"
        @click="openDeploymentLogsDialog(props.container)"
      >
        <Logs />
        Deployment Logs
      </Button>
    ...
    <DialogContent class="sm:max-w-6xl">
      <DialogHeader>
        ...
        <DialogDescription
          >Deployment logs for worker {{ props.container.worker.hostname }}</DialogDescription
        >
      </DialogHeader>
      <FieldSet>
        <FieldGroup>
          <Field />
          <Field>
            <FieldLabel for="deployment-select"> Select Deployment </FieldLabel>
            <Select v-model="selectedDeploymentId" :disabled="selectedDeploymentId === ''">
              ...
              <SelectContent>
                ...
                >
                  {{ formatDate(deployment.created_at) }} - {{ deployment.type }} -
                  {{ deployment.container_task_id }}
                </SelectItem>
              </SelectContent>
            </Select>
          </Field>
      ...
      <div class="flex flex-col max-h-[60vh]">
        <LogViewer
          ...
          container="sage"
          :search="selectedDeploymentId"
          :parseMessage="parseMessage"
          :columns="columns"
          :pollInterval="2_000"
          :poll="deploymentLogsPollCondiiton"
