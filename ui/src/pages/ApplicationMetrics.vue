...
async function load() {
  try {
    data.value = []
    const containersData = await applicationAPI.action(`${appName}/metrics?period=${period.value}`) as Array<{hostname: string, metrics: Array<ContainerMetricsPoint>}>;
...
const containerChartData = computed(() => processContainerData((data.value ?? []) as Array<Array<ContainerMetricsPoint>>))
</script>
    ...
    <div class="mx-auto space-y-6 max-w-7xl">
      ...
      <div v-else class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4">
          <MetricChart title="CPU Usage" type="line" :data="containerChartData.data.map(d => {
            const { date, ...rest } = d
          ...
          <MetricChart title="Memory Usage" type="line" :data="containerChartData.data.map(d => {
            const { date, ...rest } = d
          ...
          <MetricChart title="Network" type="line" :data="containerChartData.data.map(d => {
            const { date, ...rest } = d
