            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                ...
                <p class="text-sm text-muted-foreground mt-1">
                  {{ data?.meta?.ip }} • {{ data?.meta?.cpu_cores }} cores •
                  {{ data?.meta?.mem_total_mb }} MB RAM • {{ data?.meta?.disk_total_gb }} GB Disk
                </p>
              </div>
        ...
        <template v-if="data?.host.length">
          <div class="grid grid-cols-1 gap-4">
            <MetricChart
              ...
              type="area"
              :data="
                chartData.map(({ date, cpu_pct, cpu_pct_label }) => ({
                  date,
                  cpu_pct,
                  cpu_pct_label,
                }))
              ...
              :height="'300px'"
              :series="[
                { key: 'cpu_pct', tooltip_label: 'cpu_pct_label', name: 'CPU', color: '#3b82f6' },
              ]"
            />
            ...
            <MetricChart
              ...
              :data="
                chartData.map(
                  ({ date, mem_used, mem_used_label, mem_cached, mem_cached_label }) => ({
                    date,
                    mem_used,
                    mem_used_label,
                    mem_cached,
              ...
              "
              :yMax="data.meta.mem_total_mb"
              unit="MB"
              :height="'300px'"
              :series="[
                {
                  key: 'mem_cached',
                  tooltip_label: 'mem_cached_label',
                  name: 'Cached',
                  color: '#047857',
                },
                {
                  key: 'mem_used',
                  tooltip_label: 'mem_used_label',
                  name: 'Used',
                  color: '#10b981',
                },
              ]"
            ...
            <MetricChart
              ...
              type="area"
              :data="
                chartData.map(({ date, net_rx, net_rx_label, net_tx, net_tx_label }) => ({
                  date,
                  net_rx,
                  net_rx_label,
                  net_tx,
            ...
            <MetricChart
              ...
              type="area"
              :data="
                chartData.map(({ date, disk_gb, disk_gb_label }) => ({
                  date,
                  disk_gb,
                  disk_gb_label,
                }))
              "
              :yMax="data.meta.disk_total_gb"
              unit="GB"
              :height="'300px'"
              :series="[
                { key: 'disk_gb', tooltip_label: 'disk_gb_label', name: 'Used', color: '#06b6d4' },
              ]"
            />
