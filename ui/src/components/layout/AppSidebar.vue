  ...
  <Sidebar v-bind="props">
    ...
    <SidebarContent>
      <SidebarGroup v-for="item in menu" :key="item.name">
        ...
        <SidebarMenu>
          <SidebarMenuItem v-for="child in Array.isArray(item.children) ? item.children : [item]"
            :key="child.name || item.name">
            <SidebarMenuButton v-if="'path' in child" as-child
              :isActive="route.path === child.path || (child.path !== '/' && route.path.startsWith(child.path + '/'))">
            ...
            <SidebarMenuButton v-else-if="'url' in child" as-child>
              <a :href="child.url" :target="child.url?.startsWith('http') ? '_blank' : undefined"
