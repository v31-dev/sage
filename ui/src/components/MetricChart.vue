      ...
      <ChartContainer :config="chartConfig">
        <VisXYContainer :data="data" :margin="{ top: 10, bottom: 10 }" :yDomain="[0, props.yMax]">
          <template v-for="s in props.series" :key="s.key">
            <VisArea v-if="props.type == 'area'" :x="(d: ChartDataPoint) => d.date"
          ...
          <VisAxis type="x" :x="(d: ChartDataPoint) => d.date" :num-ticks="5" :tick-format="formatDateForXAxis" />

          <VisAxis type="y" :num-ticks="3" :tick-format="(d: number) => `${d.toFixed(0)} ${props.unit}`" />

          <ChartTooltip />
          <ChartCrosshair
            :template="componentToString(chartConfig, ChartTooltipContent, { labelFormatter: formatDateForTooltip })"
            :color="tooltipColor" />
        </VisXYContainer>
      </ChartContainer>
    </CardContent>
