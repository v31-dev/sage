...
from services.db import Application, Container, Database, Deployment, Worker
...
class Manager(Base):
    ...
    def sync_workers(self):
        ...
        for worker in tailscale_workers:
            existing_worker = next(
                (w for w in existing_workers if w.hostname == worker.hostname), None
            )
            if existing_worker:
                ...
                if existing_worker.ip != worker.ip:
                    ...
                    logger.info(
                        f"Worker {worker.hostname} IP changed from {existing_worker.ip} to {worker.ip}."
                    )
        ...
        for existing_worker in existing_workers:
            if (
                not any(w.hostname == existing_worker.hostname for w in tailscale_workers)
    ...
    def setup_worker(self, worker):
        ...
        try:
            ...
            self.cloudflare.create_dns_record(
                name=f"*.int.{get_env('DOMAIN')}",
                content=worker.ip,
                comment=f"{get_env('ORG')}-sage-worker-{worker.hostname}",
                type="A",
            )
            ...
            self.tailscale.sync_file(
                ...
                f"{worker_home_dir}/docker-compose.yml",
            )
            self.tailscale.sync_file(
                worker.hostname,
                app_dir / "templates/worker/worker.env",
                f"{worker_home_dir}/.env",
                {
            ...
            self.tailscale.sync_file(
                ...
                f"{worker_home_dir}/traefik/traefik.yml",
                {"ADMIN_EMAIL": get_env("ADMIN_EMAIL")},
            )
            self.tailscale.sync_file(
                worker.hostname,
                app_dir / "templates/worker/traefik/config.yml",
                f"{worker_home_dir}/traefik/dynamic/config.yml",
                {"DOMAIN": get_env("DOMAIN"), "HOSTNAME": worker.hostname},
            )
            self.tailscale.sync_file(
                worker.hostname,
                app_dir / "templates/worker/vector/vector.yml",
                f"{worker_home_dir}/vector/config/vector.yml",
        ...
        for container in containers:
            ...
            if status:
                if status == "running" and container.status != "active":
                    ...
                    container.save()
                    logger.error(
                        f"Marking application container {worker_container_name} as active as container is running."
                    )
                elif status in ["paused", "restarting"] and container.status != "error":
                    container.status = "error"
                    container.save()
                    logger.error(
                        f"Marking application container {worker_container_name} as error as container is in state {status}."
                    )
            else:
                ...
                if container.status == "active":
                    ...
                    container.save()
                    logger.error(
                        f"Marking application container {worker_container_name} as error as status not found (worker may be offline or container stopped)."
                    )
        ...
        for application in applications:
            ...
            if any(c.status == "error" for c in containers):
                if application.status != "error":
                    ...
                    application.save()
                    logger.error(
                        f"Marking application {application.name} as error as at least one container is in error state."
                    )
            ...
            if all(c.status == "active" for c in containers):
                if application.status != "active":
                    ...
                    application.save()
                    logger.info(
                        f"Marking application {application.name} as active as all containers are active."
                    )
            ...
            if all(c.status == "inactive" for c in containers):
                if application.status != "inactive":
                    ...
                    application.save()
                    logger.info(
                        f"Marking application {application.name} as inactive as all containers are inactive."
                    )
    ...
    def sync_application_traefik_domains_config(self, application: Application):
        ...
        logger.info(
            f"Syncing Traefik config for application {container_name} to these workers {[container.worker.hostname for container in active_containers]}."
        )
        ...
        for container in containers:
            ...
            if not container.worker.online:
                logger.error(
                    f"Worker {container.worker.hostname} is offline, skipping Traefik config sync for application {container_name} on this worker."
                )
            ...
            # Clear any config on this worker
                ...
                self.tailscale.exec_command(
                    ...
                    f"rm -f {worker_home_dir}/traefik/dynamic/{container_name}-*.yml",
                )
                logger.error(
                    f"Worker {container.worker.hostname} container is not active, skipping Traefik config sync for application {container_name} on this worker."
                )

        # Application is not active (maybe during deploying/stopping)
        if application.status != "active":
            logger.error(
                f"Worker {container.worker.hostname} container is active but application status is {application.status}, skipping Traefik config sync for application {container_name} on this worker."
            )
            return
        ...
        for container in active_containers:
            ...
            for domain in application.domains:
                traefik_config_template = (
                    "service_internal.yml" if domain.type == "internal" else "service_public.yml"
                )
                ...
                load_balanced_servers = [
                    (
                        f'{{ url: "http://traefik:9002" }}'
                        if container.worker.hostname == c.worker.hostname
    ...
    async def deploy_application(self, application: Application):
        ...
        await asyncio.gather(
            *[self.deploy_application_container(container) for container in application.containers],
    ...
    def delete_container(self, container: Container):
        ...
        container.save()
        logger.info(
            f"Deleting container of application {container.application.name} from worker {container.worker.hostname}"
        )
        ...
        try:
            ...
            container.save()
            logger.error(
                f"Failed to delete container {container.id} of application {container.application.name} from worker {container.worker.hostname}: {e}"
            )
            raise Exception(
                f"Failed to delete container {container.id} of application {container.application.name} from worker {container.worker.hostname}: {e}"
            )
    ...
    async def stop_application_container(self, container: Container):
        ...
        container.save()
        logger.info(
            f"Stopping application {container.application.name} container on worker {container.worker.hostname} with task id {container_task_id}..."
        )
        ...
        try:
            ...
            if container.status == "inactive":
                logger.info(
                    f"Container {container.id} of application {container.application.name} on worker {container.worker.hostname} is already inactive. Skipping compose down."
                )
            else:
        ...
        if deployment_status == "inactive":
            logger.info(
                f"Application {container.application.name} container stopped on worker {container.worker.hostname} with task id {container_task_id}."
            )
        else:
            logger.error(
                f"Failed to stop application {container.application.name} container on worker {container.worker.hostname} with task id {container_task_id}. Exception: {exception_message}"
            )
    ...
    async def deploy_application_container(self, container: Container):
        ...
        container.save()
        logger.info(
            f"Deploying application {container.application.name} container to worker {container.worker.hostname} with task id {container_task_id}..."
        )
        ...
        if deployment_status == "active":
            logger.info(
                f"Application {container.application.name} container deployed to worker {container.worker.hostname} with task id {container_task_id}."
            )
        else:
            logger.error(
                f"Failed to deploy application {container.application.name} container to worker {container.worker.hostname} with task id {container_task_id}. Exception: {exception_message}"
            )
