...
class Metrics(Base):
    ...
    def collect(self, ip, hostname):
        ...
        try:
            ...
            logger.info(
                f"Collected metrics for {hostname} ({len(data['containers'])} containers) at {ts.isoformat()}."
            )
    ...
    def cleanup(self, days: int = 7):
        ...
        logger.info(
            f"Metrics cleanup: removed {deleted_w} worker rows, {deleted_c} container rows older than {days} days."
        )
        ...
        for container, db_info in self._dbs["logs"].items():
            ...
            deleted_l += (
                ContainerLogs.delete().where(ContainerLogs.ts < cutoff.isoformat()).execute()
            )
