...
from utils.common import dict_to_obj, get_env
...
class Cloudflare(Base):
    def __init__(self):
        ...
        self.create_dns_record(
            ...
            comment=f"{self.org}-sage-manager",
            type="A",
        )
    ...
    def create_dns_record(self, name, content, comment=None, *args, **kwargs):
        ...
        if existing_record:
            ...
            if existing_record.content != content:
                logger.info(
                    f"DNS record for {name} exists but content is {existing_record.content}. Updating content to {content}."
                )
