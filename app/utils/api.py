...
def get_request_models(
  ...
  """Return the shared request-scoped model cache for nested routes.

  When `required` keys are provided, the helper raises 404 if any key is
  missing. By default, required keys must also have a non-None value.
  """
  ...
  for key in required or []:
    if key not in models or (models[key] is None and not none_allowed):
...
def generic_list(model, query_condition=None):
  ...
  try:
    ...
    raise HTTPException(
        status_code=500, detail=f"Failed to list {model.__name__} with error: {e}"
    )
...
def generic_create(model, data: dict):
  ...
  try:
    ...
    raise HTTPException(
        status_code=500, detail=f"Failed to create {model.__name__} with error: {e}"
    )
...
def generic_get(model, query_condition, return_model=False):
  ...
  try:
    ...
    raise HTTPException(
        status_code=500, detail=f"Failed to fetch {model.__name__} with error: {e}"
    )
...
def generic_update(model, instance, data: dict):
  ...
  try:
    ...
    raise HTTPException(
        status_code=500, detail=f"Failed to update {model.__name__} with error: {e}"
    )
...
def generic_delete(model, instance):
  ...
  try:
    ...
    raise HTTPException(
        status_code=500, detail=f"Failed to delete {model.__name__} with error: {e}"
    )
