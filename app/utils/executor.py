...
# Every thread pool in the process is defined here. The first five are the
# operation-queue lanes (wired into TaskQueue by the Manager); each scope root
# gets its own pool -- platform/common/metrics/notifications one worker each
# (metrics two on bigger hosts), app the rest.
...
# Notifications: the daily-summary queue lane and inline webhook fire-and-forget
# sends share this single-writer pool.
