...
from utils.api import generic_create, generic_get, generic_list
...
def inject_container(request: Request):
    ...
    if not worker:
        raise HTTPException(
            status_code=404, detail=f"Worker with hostname '{container_name}' not found"
        )

    request.state.models["container"] = generic_get(
        Container,
