...
def inject_domain(request: Request):
  ...
  # Fetch and store domain
  domain_name = request.path_params.get('domain')
...
def create_domain(request: Request, domain_data: dict = Body(...)):
  data = {
    ...
    'name': domain_data.get('name').lower().strip(),
    'type': domain_data.get('type').lower().strip()
  }
...
def update_domain(request: Request, domain_data: dict = Body(...)):
  data = parse_api_data(domain_data, ['name', 'type'])
